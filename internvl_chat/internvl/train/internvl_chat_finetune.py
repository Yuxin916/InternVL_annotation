...
"""
If you want to debug this script and inspect variables: 
1. Set arguments: 
    ../../configs_finetune/lora_70k_few.json
2. Set environment variables:
    TF_CPP_MIN_LOG_LEVEL=3;MASTER_PORT=34229;IS_DEBUG=1;GPUS=1;MASTER_ADDR=127.0.0.1;
3. Working directory:
...
def main():
    ...
    if os.getenv("IS_DEBUG", "0") == "1":
        try:
            ...
            logging.info("Successfully initialized process group for debugging.")
        except Exception as e:
            logging.error(f"Failed to initialize process group: {e}")
    else:
        logging.info('Initializing distributed training as original script...')
